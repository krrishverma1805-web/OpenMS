...
namespace OpenMS
{
  ...
  bool INIFileEditorWindow::openFile(const String& filename)
  {
    if (filename.empty())
    {
      filename_ = QFileDialog::getOpenFileName(this, tr("Open ini file"), QString::fromStdString(static_cast<const std::string&>(current_path_)), tr("ini files (*.ini);; all files (*.*)"));
    }
    else
    {
  ...
  {
    filename_ = QFileDialog::getSaveFileName(this, tr("Save ini file"), QString::fromStdString(static_cast<const std::string&>(current_path_)), tr("ini files (*.ini)"));
    if (!filename_.isEmpty())
    {
  ...
  void INIFileEditorWindow::updateWindowTitle(bool update)
  {
    ...
    if (update)
    {
      setWindowTitle(QString::fromStdString(static_cast<const std::string&>(File::basename(String(filename_.toStdString())) + " * - INIFileEditor")));
    }
    else
    {
      setWindowTitle(QString::fromStdString(static_cast<const std::string&>(File::basename(String(filename_.toStdString())) + " - INIFileEditor")));
    ...
    current_path_ = File::path(String(filename_.toStdString()));
  }
