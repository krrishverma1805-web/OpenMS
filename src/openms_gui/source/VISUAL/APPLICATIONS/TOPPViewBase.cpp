...
namespace OpenMS
{
    ...
    menu_(this, &ws_, &recent_files_)
  {
    ...
    {
      QAction* temp = group_unassigned_2d_->addAction(QString::fromStdString(static_cast<const std::string&>(opt)));
      temp->setCheckable(true);
    ...
    // FileWatcher::fileChanged signal not available in core Qt-free build; change monitoring is disabled.
  }
      ...
      else if (file_type == FileTypes::IDXML || file_type == FileTypes::MZIDENTML)
      {
        ...
        if (!proteins.empty())
        {
          ...
          if (!annotate_path.empty())
          {
            ...
            msg_box.setInformativeText(QString::fromStdString(static_cast<const std::string&>(String("Annotate spectra in " + spectra_file_name + "?"))));
  ...
  void TOPPViewBase::showCursorStatus(const String& x, const String& y)
  {
    ...
    x_label_->setText(QString::fromStdString(static_cast<const std::string&>(x)));
    y_label_->setText(QString::fromStdString(static_cast<const std::string&>(y)));
    statusBar()->update();
  }
  ...
  void TOPPViewBase::showPlotWidgetInWindow(PlotWidget* sw)
  {
    ...
    sw->setWindowTitle(QString::fromStdString(static_cast<const std::string&>(base_name)));
  ...
  {
    QString open_path = QString::fromStdString(static_cast<const std::string&>(current_path_));
    if (!path_overwrite.empty())
    {
      open_path = QString::fromStdString(static_cast<const std::string&>(path_overwrite));
    }
    ...
    QFileDialog dialog(this, "Open file(s)", open_path, QString::fromStdString(static_cast<const std::string&>(supported_types.toFileDialogFilter(FilterLayout::BOTH, true))));
  ...
  void TOPPViewBase::openFilesByDialog(const String& dir)
  {
    ...
    {
      addDataFile(String(filename.toStdString()), true, true);
    }
  }
  ...
  void TOPPViewBase::runTOPPTool_()
  {
    ...
    QStringList args;
    args << "-ini"
         << QString::fromStdString(static_cast<const std::string&>(topp_.file_name + "_ini"))
         << QString("-%1").arg(QString::fromStdString(static_cast<const std::string&>(topp_.in)))
         << QString::fromStdString(static_cast<const std::string&>(topp_.file_name_in))
         << "-no_progress";
    if (topp_.out != "")
    {
      args << QString("-%1").arg(QString::fromStdString(static_cast<const std::string&>(topp_.out)))
           << QString::fromStdString(static_cast<const std::string&>(topp_.file_name_out));
    }

    // start log and show it
    log_->appendNewHeader(
      LogWindow::LogState::NOTICE,
      String(QString("Starting '%1'").arg(QString::fromStdString(static_cast<const std::string&>(topp_.tool))).toStdString()),
      String(""));
    ...
    QString tool_executable = QString::fromStdString(static_cast<const std::string&>(tool_scanner_.findPluginExecutable(topp_.tool)));
    if (tool_executable.isEmpty())
    {
      try
      {
        // find correct location of TOPP tool
        tool_executable = QString::fromStdString(static_cast<const std::string&>(File::findSiblingTOPPExecutable(topp_.tool)));
      }
      catch (Exception::FileNotFound & /*ex*/)
      {
        log_->appendNewHeader(
          LogWindow::LogState::CRITICAL,
          String("Could not locate executable!"),
          String(QString("Finding executable of TOPP tool '%1' failed. Please check your TOPP/OpenMS installation. Workaround: Add the bin/ directory to your PATH")
                   .arg(QString::fromStdString(static_cast<const std::string&>(topp_.tool)))
                   .toStdString()));
        return;
      }
    }
    ...
    if (topp_.process->error() == QProcess::FailedToStart)
    {
      log_->appendNewHeader(
        LogWindow::LogState::CRITICAL,
        String(QString("Failed to execute '%1'")
                 .arg(QString::fromStdString(static_cast<const std::string&>(topp_.tool))).toStdString()),
        String(QString("Execution of TOPP tool '%1' failed with error: %2")
                 .arg(QString::fromStdString(static_cast<const std::string&>(topp_.tool)), topp_.process->errorString())
                 .toStdString()));
  ...
  void TOPPViewBase::finishTOPPToolExecution(int, QProcess::ExitStatus)
  {
    ...
    if (topp_.process->exitStatus() == QProcess::CrashExit)
    {
      log_->appendNewHeader(
        LogWindow::LogState::CRITICAL,
        String(QString("Execution of '%1' not successful!").arg(QString::fromStdString(static_cast<const std::string&>(topp_.tool))).toStdString()),
        String(QString("The tool crashed during execution. If you want to debug this crash, check the input files in '%1'"
                       " or enable 'debug' mode in the TOPP ini file.").arg(QString::fromStdString(static_cast<const std::string&>(File::getTempDirectory()))).toStdString()));
    }
    else if (topp_.process->exitCode() != 0) // NormalExit with non-zero exit code
    {
      log_->appendNewHeader(
        LogWindow::LogState::CRITICAL,
        String(QString("Execution of '%1' not successful!").arg(QString::fromStdString(static_cast<const std::string&>(topp_.tool))).toStdString()),
        String((QString("The tool ended with a non-zero exit code of '%1'. ").arg(topp_.process->exitCode()) +
                QString("If you want to debug this, check the input files in '%1' or"
                        " enable 'debug' mode in the TOPP ini file.").arg(QString::fromStdString(static_cast<const std::string&>(File::getTempDirectory())))).toStdString()));
    }
    else if (!topp_.out.empty())
    {
      log_->appendNewHeader(
        LogWindow::LogState::NOTICE,
        String(QString("'%1' finished successfully").arg(QString::fromStdString(static_cast<const std::string&>(topp_.tool))).toStdString()),
        String(QString("Execution time: %1 ms").arg(topp_.timer.elapsed()).toStdString()));
  ...
  void TOPPViewBase::loadFiles(const StringList& list, QSplashScreen* splash_screen)
  {
    ...
    for (StringList::const_iterator it = list.begin(); it != list.end(); ++it)
    {
      ...
      splash_screen->showMessage(QString::fromStdString(static_cast<const std::string&>(String("Loading file: ") + *it)));
      ...
      else if (source == nullptr)
      {
        ...
        if (data->hasUrls())
        {
          ...
          QTimer::singleShot(50, [this, urls, new_id]() {
            ...
            {
              addDataFile(String(url.toLocalFile().toStdString()), false, true, "", new_id);
            }
          });
        }
      }
  ...
  void TOPPViewBase::fileChanged_(const String& filename)
  {
    ...
    else if (param_.getValue(user_section + "on_file_change") == "ask") // ask the user if the layer should be updated
    {
      ...
      msg_box.setText(QString::fromStdString(static_cast<const std::string&>(String("The data of file '") + filename + "' has changed.<BR>Update layers?")));
      msg_box.exec();
    ...
    if (auto* lp = dynamic_cast<LayerDataPeak*>(&layer)) // peak data
    {
      ...
      {
        QMessageBox::critical(this, "Error", QString::fromStdString(static_cast<const std::string&>(String("Error while loading file") + layer.filename + "\nError message: " + e.what())));
