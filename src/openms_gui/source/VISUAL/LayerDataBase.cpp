...
*/
  ...
  bool LayerAnnotatorBase::annotateWithFileDialog(LayerDataBase& layer, LogWindow& log, const String& current_path) const
  {
    ...
    QString fname = QFileDialog::getOpenFileName(nullptr,
                                                 QString::fromStdString(static_cast<const std::string&>(file_dialog_text_)),
                                                 QString::fromStdString(static_cast<const std::string&>(current_path)),
                                                 QString::fromStdString(static_cast<const std::string&>(supported_types_.toFileDialogFilter(FilterLayout::BOTH, true))));

    bool success = annotateWithFilename(layer, log, OpenMS::String(fname.toStdString()));
  ...
  bool LayerAnnotatorBase::annotateWithFilename(LayerDataBase& layer, LogWindow& log, const String& fname) const
  {
    ...
    {
      {
        std::string msg = std::string("Filename '") + static_cast<const std::string&>(fname) + "' has unsupported file type. No annotation performed.";
        log.appendNewHeader(LogWindow::LogState::NOTICE, "Error", OpenMS::String(msg));
      }
      return false;
    }
  ...
  bool LayerAnnotatorAMS::annotateWorker_(LayerDataBase& layer, const String& filename, LogWindow& log) const
  {
    ...
    {
      QString msg = QString("FeatureXML is currently only supported for files generated by the AccurateMassSearch tool (got '%1', expected 'AccurateMassSearch'.)")
                      .arg(QString::fromStdString(static_cast<const std::string&>(engine)));
