...
namespace OpenMS
{
  ...
  void TOPPASOutputFolderVertex::run()
  {
    ...
    for (Size round = 0; round < round_total_; ++round)
    {
      ...
      {
        if (!dry_run && !File::isDirectory(OpenMS::String(src_dir.toStdString())))
        {
        ...
        output_files_[round][param_index_me].filenames.push_back(QString::fromStdString(static_cast<const std::string&>(new_dir)));
        // find number of files in 'src_dir'
        QDir dir(src_dir);
        auto nr_of_files = dir.entryInfoList(QDir::Files).size();
        if (!dry_run && nr_of_files == 0)
        { 
          String msg = "Output directory '" + String(e->getSourceOutParamName().toStdString()) + "' did not yield any files!";
          if (ts->isGUIMode()) 
          {
            QMessageBox::warning(nullptr, tr("No files found"), QString::fromStdString(static_cast<const std::string&>(msg)), QMessageBox::Ok);
          }
          else
          {
    ...
    else
    {
      for (Size round = 0; round < pkg.size(); ++round)
      {
        ...
        {
          String dir_from = String(pkg[round][param_index_src].filenames[i].toStdString());
          String dir_to   = String(output_files_[round][param_index_me].filenames[i].toStdString());
          ...
          const auto& src_files_to_copy = QDir(QString::fromStdString(static_cast<const std::string&>(dir_from))).entryInfoList(QDir::Files);
          for (const auto& src_file : src_files_to_copy)
          {
            ...
            if (File::exists(file_to) // someone may have deleted the file in the meantime, which is fine
              && !QFile::remove(QString::fromStdString(static_cast<const std::string&>(file_to)))) // remove old file (would fail if file does not exist)
            {
