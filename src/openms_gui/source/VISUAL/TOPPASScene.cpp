...
namespace OpenMS
{
  ...
  TOPPASScene::TOPPASScene(QObject* parent, const QString& tmp_path, bool gui) :
    ...
    tmp_path_(tmp_path),
    gui_(gui),
    out_dir_(QString::fromStdString(static_cast<const std::string&>(File::getUserDirectory()))),
    changed_(false),
    running_(false),
    ...
    if (target && target != hover_edge_->getSourceVertex())
    {
      ...
      if ((QGuiApplication::keyboardModifiers() & Qt::ControlModifier) && tv_source)
      {
        ...
        else
        {
          ...
          emit messageReady(QString::fromStdString(ss.str()));
          //std::cerr << ss.str();
        }
        remove_edge = true;
      }
  ...
  bool TOPPASScene::store(const String& file)
  {
    ...
    save_param.setValue("info:description", String("<![CDATA[") + String(this->description_text_.toStdString()) + String("]]>"));
    ...
    for (TOPPASVertex * tv : vertices_)
    {
      ...
      if (auto* iflv = qobject_cast<TOPPASInputFileListVertex*>(tv); iflv)
      {
        ...
        QDir save_dir(QString::fromStdString(static_cast<const std::string&>(File::path(file))));
  ...
  void TOPPASScene::load(const String& file)
  {
    ...
    if (load_param.exists("info:description"))
    {
      ...
      text.substitute("]]>", "");
      description_text_ = QString::fromStdString(static_cast<const std::string&>(text.trim()));
    }
    ...
    for (Param::ParamIterator it = vertices_param.begin(); it != vertices_param.end(); ++it)
    {
      ...
      if (substrings.back() == "toppas_type") // next node (all nodes have a "toppas_type")
      {
        ...
        if (current_type == "input file list")
        {
          ...
          {
            QString f = QString::fromStdString(static_cast<const std::string&>(*str_it));
            if (QDir::isRelativePath(f)) // prepend path of toppas file to relative path of the input files
            {
              f = QString::fromStdString(static_cast<const std::string&>(File::path(file))) + "/" + f;
            }
        ...
        else if (current_type == "output file list")
        {
          ...
          {
            oflv->setOutputFolderName(QString::fromStdString(static_cast<const std::string&>(String(vertices_param.getValue(current_id + ":output_folder_name").toString()))));
          }
        ...
        else if (current_type == "output folder")
        {
          ...
          if (vertices_param.exists(current_id + ":output_folder_name"))
          {
            ofv->setOutputFolderName(QString::fromStdString(static_cast<const std::string&>(String(vertices_param.getValue(current_id + ":output_folder_name").toString()))));
          }
      ...
      else
      {
        ...
        else
        {
          ...
          if (source_out_param != "__no_name__" && tv_src)
          {
            ...
            if (src_index == -1)
              logTOPPOutput(QString::fromStdString(static_cast<const std::string&>(String("Could not find output parameter called '" + source_out_param + "'. Check edge!"))));
          }
          ...
          if (target_in_param != "__no_name__" && tv_src)
          {
            ...
            if (tgt_index == -1)
              logTOPPOutput(QString::fromStdString(static_cast<const std::string&>(String("Could not find input parameter called '" + target_in_param + "'. Check edge!"))));
          }
...
*/
  ...
  void TOPPASScene::writeToLogFile_(const QString& text)
  {
    ...
    {
      std::cerr << "Could not write to logfile '" << String(logfile.fileName().toStdString()) << "'" << std::endl;
      return;
    }
  ...
  void TOPPASScene::logTOPPOutput(const QString& out)
  {
    ...
    String text = String(out.toStdString());
    ...
    writeToLogFile_(QString::fromStdString(static_cast<const std::string&>(text)));
  }
  ...
  bool TOPPASScene::saveIfChanged()
  {
    ...
    if (gui_ && changed_)
    {
      QString name = file_name_.empty() ? "Untitled" : QString::fromStdString(static_cast<const std::string&>(File::basename(file_name_)));
