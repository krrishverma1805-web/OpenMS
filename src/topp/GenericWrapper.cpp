  ...
  */
  String paramToString_(const Param::ParamEntry & p)
  {
    ...
    if (p.value.valueType() == ParamValue::STRING_LIST) // quote each element
    {
      ...
      if (p.tags.count("input file") || p.tags.count("output file"))
      {
        ...
        {
          val[i] = String(QDir::toNativeSeparators(QString::fromStdString(val[i])).toStdString());
        }
      }
    ...
    if (p.tags.count("input file") || p.tags.count("output file"))
    {
      ...
      return String(QDir::toNativeSeparators(QString::fromStdString(String(p.value.toString()))).toStdString());
    }
    else
    {
  ...
  void createFragment_(String & fragment, const Param & param, const std::map<int, std::string>& optional_mappings = (std::map<int, std::string>()))
  {
    ...
    // %DIR% replace
    {
      ...
      QString t_tmp = QString::fromStdString(static_cast<const std::string&>(fragment));
      //std::cout << "fragment is:" << fragment << std::endl;
      for (const QRegularExpressionMatch& match : rx.globalMatch(QString::fromStdString(static_cast<const std::string&>(fragment))))
      {
        String value = String(match.captured(1).toStdString());   // param name (hopefully)
        // replace in fragment:
      ...
      fragment = String(t_tmp.toStdString());
