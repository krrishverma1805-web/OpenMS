  ...
  ExitCodes main_(int, const char**) override
  {
    ...
    if (show_precursors)
    {
      markMS2Locations_(exp, image, getFlag_("transpose"),
                        QString::fromStdString(static_cast<const std::string&>(getStringOption_("precursor_color"))),
    ...
    if (image.save(QString::fromStdString(static_cast<const std::string&>(out)), format.c_str())) return EXECUTION_OK;
